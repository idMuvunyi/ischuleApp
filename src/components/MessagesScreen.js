...
const MessagesScreen = ({userDetails}) => {
            ...
            <View style={styles.namesDate}>
              <Text style={{color: COLORS.primary, flex: 1}}>
                {userInfo[0].userType === 'tutor'
                  ? item.senderName.split(' ')[1]
                  : item.receiverName.split(' ')[1]}{' '}
                :
              </Text>
  ...
  const NoMessageAvailable = () => {
    return (
      <View
        style={{alignItems: 'center', color: COLORS.textColor, marginTop: 50}}>
  ...
  return (
    <View style={styles.container}>
      {loading ? (
        ...
        <FlatList
          ...
          renderItem={({item, index}) => (
            <MessageComponent item={item} id={index} />
          )}
      ...
      <Modal
        ...
        footer={
          <ModalFooter>
            <ModalButton text="Call" onPress={() => handleCallEmployer()} />
          ...
          <View>
            <Text
              style={{
                fontSize: 15,
                lineHeight: 22,
              }}>{`You are about to call employer ${modalData.senderName} for professional tutoring purpose.`}</Text>
          </View>
        </ModalContent>
      </Modal>
    </View>
